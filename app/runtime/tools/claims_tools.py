...
class ClaimsTools:
    ...
    def __init__(
        ...
        logger.info(
            f"Claims tools initialized for tenant {self.tenant_id} with auth token {self.auth_token}"
        )
...
def get_claims_tools(
    tenant_id: str, backend_url: Optional[str] = None, auth_token: Optional[str] = None
) -> ClaimsTools:
    """
    ...
    """
    # Create a unique key that includes auth_token to allow different tokens per tenant
    cache_key = f"{tenant_id}:{auth_token or 'no-token'}"
    if cache_key not in _claims_tools_instances:
        _claims_tools_instances[cache_key] = ClaimsTools(
            tenant_id, backend_url, auth_token
        )
    else:
